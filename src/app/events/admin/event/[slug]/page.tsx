...
export default function AdminPage() {
  ...
  // Slug is driven by the URL; we keep it in state so existing callbacks can
  // continue to read `selectedSlug` without churn.
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Pass slug so the API resolves it via getEventBySlugAnyStatus and
      // returns past (inactive) events too.
      const res = await fetch(`/api/events/admin?slug=${urlSlug}`);
  ...
  // Keep selectedSlug in sync with the URL slug. The legacy auto-select
  // logic (pick events[0] when nothing selected) is gone — slug now comes
  ...
  const addSingleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.duplicates > 0) {
        ...
        // Background sync to reconcile with sheet
        setTimeout(() => fetchInvites(), 1500);
      }
    } catch {
  ...
  // (Event picker removed — slug now comes from the URL via /events/admin/event/[slug].
  // Header chrome lives in the admin layout's <AdminHeader />.)
      ...
      <header className="bg-brand-navy text-white no-print">
        <div className="max-w-content mx-auto px-6 md:px-10 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-[11px] font-sans font-semibold tracking-wider uppercase text-white/40">
              {events[0]?.name || "Loading..."}
            </span>
          </div>
        </div>
      </header>
          ...
          <>
            ...
            {(() => {
              ...
              const exportAttendeesPdf = () => {
                ...
                // Combine confirmed + pending into one list
                const allAttendees = [
                  ...confirmedRows.map((r) => ({
                    name: [r.firstName, r.surname].filter(Boolean).join(" ") || "—",
                    title: r.title || "—",
